...
// pure helper functions
    ...
    function HW2() {
        ...
        const [undoState, setUndoState] = useState<Array<AffairType>>(defaultAffairs) // делаю стэйт для отката последнего действия

        const filteredAffairs = filterAffairs(affairs, filter)

        const deleteAffairCallback = (_id: number) => {
            setUndoState(affairs)
            setAffairs(deleteAffair(affairs, _id))
        } // need to fix any

        const setLastState = () => {
            setAffairs(undoState)
        }
